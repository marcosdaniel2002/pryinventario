...
export function CategoriasTable({ token }: { token: string | null }) {
  const [page, setPage] = useState(1);
  const { data: response, isLoading, isError } = useCategoriasPaginated(
    token,
    page,
    DEFAULT_PAGE_SIZE,
  );
  ...
  const deleteRegistro = useDeleteCategoria(token);

  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingRegistro, setEditingRegistro] =
    useState<CategoriaListItem | null>(null);
  ...
  function openEditDialog(registro: CategoriaListItem) {
        ...
        <div className="overflow-x-auto rounded-lg border border-border">
          <table className="w-full text-left text-sm">
            <thead className="border-b border-border bg-muted/50">
              <tr>
                {/* Agregá o quitá columnas acá segun lo que necesites mostrar. */}
